"""
Agente Monitor — saúde e alertas da squad (skill definida em applications/agents).
Uso: python -m orchestrator.agents.monitor --input message.json [--skill-path monitor/backend]
     ou POST /invoke/monitor com body message_envelope (skill_path opcional em input.context.skill_path).
"""
import argparse
import json
import logging
...
DEFAULT_SKILL_PATH = "monitor/backend"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Monitor Agent — health e alertas (skill em applications/agents)")
    ...
    parser.add_argument("--skill-path", help="Skill path relativo a applications/agents (default: monitor/backend)", default=None)
    ...
    if "request_id" not in message:
        message["request_id"] = "monitor-cli"
    ...
    logger.info("Chamando agente Monitor (SYSTEM_PROMPT: %s)", prompt_path)
    response = run_agent(
