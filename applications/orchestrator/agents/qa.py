"""
Agente QA — testes e validação (skill definida em applications/agents).
Uso: python -m orchestrator.agents.qa --input message.json [--skill-path qa/backend/nodejs]
     ou POST /invoke/qa com body message_envelope (skill_path opcional em input.context.skill_path).
"""
import argparse
import json
import logging
...
DEFAULT_SKILL_PATH = "qa/backend/nodejs"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="QA Agent — testes e validação (skill em applications/agents)")
    ...
    logger.info("Chamando agente QA (SYSTEM_PROMPT: %s)", prompt_path)
    response = run_agent(
