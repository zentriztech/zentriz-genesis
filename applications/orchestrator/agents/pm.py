"""
Agente PM — backlog e gestão da squad (skill definida em applications/agents).
Uso: python -m orchestrator.agents.pm --input message.json [--skill-path pm/backend]
     ou POST /invoke/pm com body message_envelope (skill_path opcional em input.context.skill_path).
"""
import argparse
import json
import logging
...
def main() -> int:
    parser = argparse.ArgumentParser(description="PM Agent — backlog e gestão (skill em applications/agents)")
    ...
    logger.info("Chamando agente PM (SYSTEM_PROMPT: %s)", prompt_path)
    response = run_agent(
