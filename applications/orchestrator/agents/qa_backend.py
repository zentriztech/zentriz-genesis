...
SYSTEM_PROMPT_PATH = REPO_ROOT / "agents" / "qa" / "backend" / "nodejs" / "SYSTEM_PROMPT.md"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="QA Backend Agent — testes e validação Backend (Node.js/TypeScript)")
    ...
    if "request_id" not in message:
        message["request_id"] = "qa-backend-cli"
    if "input" not in message:
        message["input"] = message.get("context", {})

    logger.info("Chamando agente QA Backend (SYSTEM_PROMPT: %s)", SYSTEM_PROMPT_PATH)
    response = run_agent(
