"""
Agente Dev — implementação (skill definida em applications/agents).
Uso: python -m orchestrator.agents.dev --input message.json [--skill-path dev/backend/nodejs]
     ou POST /invoke/dev com body message_envelope (skill_path opcional em input.context.skill_path).
"""
import argparse
import json
import logging
...
DEFAULT_SKILL_PATH = "dev/backend/nodejs"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dev Agent — implementação (skill em applications/agents)")
    ...
    logger.info("Chamando agente Dev (SYSTEM_PROMPT: %s)", prompt_path)
    response = run_agent(
