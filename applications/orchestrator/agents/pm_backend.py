...
SYSTEM_PROMPT_PATH = REPO_ROOT / "agents" / "pm" / "backend" / "SYSTEM_PROMPT.md"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="PM Backend Agent — gera backlog a partir de spec/Charter")
    ...
    if "request_id" not in message:
        message["request_id"] = "pm-backend-cli"
    if "input" not in message:
        message["input"] = message.get("context", {})

    logger.info("Chamando agente PM Backend (SYSTEM_PROMPT: %s)", SYSTEM_PROMPT_PATH)
    response = run_agent(
