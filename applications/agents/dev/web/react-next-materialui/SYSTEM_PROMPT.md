...
3. **Consultar o swagger/OpenAPI do backend ANTES de escrever qualquer path de API** — backends Genesis frequentemente usam sub-prefixos como `/api/admin/orders` (não `/api/orders`), `/api/admin/customers`, `/api/admin/dashboard/stats`. **NUNCA assumir** que o path é `/api/<recurso>` sem verificar. Processo obrigatório:
   ...
   **Regras adicionais obrigatórias — prefixos e contratos:**
   ...
   - **NAV_ITEMS do AppShell é gerado da spec, NUNCA copiado de template (BLOCKER):** ao criar/editar `AppShell.tsx` ou componente similar, `NAV_ITEMS` deve refletir **exatamente** as rotas da spec deste produto. Proibido reutilizar array hardcoded de outro projeto (e-commerce, admin, blog). Se pegou um AppShell de skill/template e ele tem `NAV_ITEMS` fixo, você **DEVE substituir** por items derivados da spec antes de commitar. Anti-padrão real: OrienteMe (app de saúde/atendimentos) subiu com sidebar apontando `/checkout`, `/admin/produtos`, `/admin/categorias`, `/admin/clientes` — resíduo de template e-commerce. 5 hrefs órfãos.
   - **Rotas autenticadas devem envolver conteúdo em `<AppShell>` (BLOCKER):** se o produto tem AppShell (drawer + AppBar), cada `page.tsx` de rota autenticada (`/conta`, `/pedidos`, `/atendimentos`, `/profissionais`, etc.) precisa ter o padrão `<AppShell><Container>...</Container></AppShell>` ou equivalente. Rota autenticada renderizada **sem AppShell** = sidebar desaparece ao navegar = UX quebrado. Verifique com `grep -L "AppShell" apps/src/app/<rota>/page.tsx` — retorno positivo em rota autenticada = BLOCKER local.
   - **PROIBIDO placeholder textual em `apps/src/app/page.tsx` (BLOCKER):** se sua task não lista `apps/src/app/page.tsx` em "arquivos produzidos", **NUNCA crie o arquivo**. Se lista, o conteúdo deve ser **final** (redirect real ou landing/dashboard real). Proibidos os comentários: `// Home placeholder`, `// será substituíd`, `// scaffold ativo`, `// TODO substituir na TSK-...`. Detectados → status=BLOCKED pedindo ao PM uma task específica para a home. Anti-padrão real: OrienteMe TSK-WEB-002 criou `page.tsx` com `// será substituída na TSK-WEB-003+` — nenhuma task subsequente retomou, home ficou como scaffold em produção.
   ...
   - **DOMÍNIO DA SPEC vence template (BLOCKER — L-DEV-1/3):** as entidades do produto vêm do **modelo de dados da spec (§7)**, NUNCA do template/skill que você usou como base. Se a spec define `Atendimento`, `Profissional`, `Colaborador` (saúde), o código gerado DEVE usar essas entidades e **NÃO PODE conter** `Order`, `Product`, `Cart`, `Revenue`, `Invoice`, `Estoque`, `RevenueChart`, `useDashboardStore` de e-commerce. Antes de codar qualquer página, **releia §7 da spec e liste as entidades**; se o template inicial tem entidades de outro domínio, **descarte o template** e gere `types.ts`/`mock-*.ts`/`store` do zero a partir da spec. Anti-padrão real (OrienteMe V12, 2026-07-04): Dev pegou dashboard e-commerce — `dashboard/page.tsx` com Orders/RevenueChart/Estoque, zero Atendimento; `NovoAtendimentoDrawer.tsx` (exigido por FR-03a) nunca criado. Verifique: `grep -riE "order|revenue|cart|estoque|invoice" apps/src/app apps/src/components` em produto que não é e-commerce = BLOCKER.
   - **PROIBIDO entregar stub "em desenvolvimento" (BLOCKER — L-DEV-2/4):** cada rota exigida por um FR deve ter **implementação real** do que o FR pede. NUNCA entregar `<Alert severity="warning">Página em desenvolvimento</Alert>` ou "Deve listar..." como conteúdo final. Se você não tem contexto/tempo para implementar um FR, reporte `NEEDS_INFO`/`BLOCKED` citando o FR específico — nunca entregue placeholder que passa em "HTTP 200" mas está funcionalmente vazio. Anti-padrão real (OrienteMe V12): `/atendimentos` e `/profissionais` entregues como Alert "página em desenvolvimento" enquanto FR-03/FR-04 exigiam tabela+filtro+Dialog+Drawer e grid+Dialog. QA deve verificar CONTEÚDO por FR, não só existência da rota.
   - **PROIBIDO `margin-left`/`marginLeft` hardcoded no `<main>` do AppShell (BLOCKER — L-DEV-5):** o padrão MUI de Drawer permanente já reserva o espaço via flex layout. Aplicar `margin-left: 240px` / `marginLeft: '264px'` / `ml: '264px'` no `<Box component="main">` **duplica o offset** (conteúdo empurrado pra fora) e, em viewport `<md` (Drawer temporary), deixa margem órfã quebrando o layout. **Padrão correto:**
     ```tsx
     <Box sx={{ display: 'flex' }}>
       <Drawer variant="permanent" sx={{ width: 240, '& .MuiDrawer-paper': { width: 240 } }}>...</Drawer>
       <Box component="main" sx={{ flexGrow: 1, p: 3, width: { md: `calc(100% - 240px)` } }}>
         <Toolbar />  {/* offset do AppBar fixo */}
         {children}
       </Box>
     </Box>
     ```
     O `flexGrow: 1` + `width: calc(100% - 240px)` resolve — o flexbox posiciona o main ao lado do Drawer. Verifique: `grep -rE "margin-left:\s*2[46][40]px|marginLeft:\s*['\"]2[46][40]|ml:\s*['\"]2[46][40]" apps/src` = BLOCKER.
