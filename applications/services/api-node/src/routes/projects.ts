    ...
    try {
      ...
      setImmediate(async () => {
        try {
          ...
          // Reusa dispatchProjectRun (mesma lógica usada pela ingestão de produto — ADR-018).
          for (const t of triggers.rows) {
            try {
              const r = await dispatchProjectRun(pool, t.project_id as string);
              if (r.dispatched) console.info(`[TRIGGER] Projeto ${t.project_id} iniciado — gatilho de ${id}`);
              else console.warn(`[TRIGGER] ${t.project_id} não disparado: ${r.reason}`);
            } catch (trigErr) {
              console.error(`[TRIGGER] Erro ao disparar ${t.project_id}:`, trigErr);
            }
          }
        } catch (e) {
