...
- **Rotas**: `GET/POST/PATCH /api/projects`, `POST /api/specs` (multipart, multi-arquivo .md/.txt/.doc/.docx/.pdf), `GET/POST /api/users`, `GET /api/tenants`. PATCH projects aceita `started_at`, `completed_at`, `status`, `charter_summary`.
