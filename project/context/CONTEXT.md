...
- **Testes**: API (Vitest); conversor (pytest); smoke test em [tests/smoke/api_smoke_test.sh](../../project/tests/smoke/api_smoke_test.sh); unit e integração em [orchestrator/tests/](../../applications/orchestrator/tests/) (pytest a partir de `applications/`, 69 testes). **Testes e2e do pipeline** (spec → CTO → … → artefato com LLM real ou mockado): não implementados; previstos para iteração futura.
