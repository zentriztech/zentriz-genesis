...
- **Testes**: API (Vitest); conversor (pytest); smoke test em [tests/smoke/api_smoke_test.sh](../../project/tests/smoke/api_smoke_test.sh); unit e integração em [orchestrator/tests/](../../applications/orchestrator/tests/) (pytest a partir de `applications/`, 69 testes). **Testes E2E do pipeline**: [tests/e2e/test_pipeline_landing.py](../../tests/e2e/test_pipeline_landing.py) conforme [E2E_PIPELINE_TEST_GUIDE.md](../docs/E2E_PIPELINE_TEST_GUIDE.md); spec [project/spec/spec_landing_zentriz.txt](../spec/spec_landing_zentriz.txt); requer agents na porta 8000 e CLAUDE_API_KEY. Registro em [context/ia/016_20260221_e2e_pipeline_landing.md](ia/016_20260221_e2e_pipeline_landing.md).
...
### Regras de comunicação agente→LLM (2026-02-21)
- **Fonte**: [docs/AGENT_LLM_COMMUNICATION_ANALYSIS.md](../docs/AGENT_LLM_COMMUNICATION_ANALYSIS.md) (análise Claude Opus). **Fase 1 (002):** Parse `<response>`, `build_user_message`, `build_system_prompt` com templates, `PipelineContext`, thinking/response em todos os SYSTEM_PROMPT, `validate_response_quality` + retry. **Continuação (003):** Protocolo compartilhado com tabela "Quando usar cada status" e "Qualidade do Output"; seção "Comunicação Permitida" em CTO, Engineer, PM, Dev, QA, Monitor; CTO com "Seu Papel e Como Pensar" e golden example completo (loja de veículos); instruções operacionais em Dev, Engineer, PM, QA; `PipelineContext.get_dependency_code()` e `register_artifact()`; `build_user_message` com suporte a `current_task`, `dependency_code` e `previous_attempt` (Dev). **004:** Runner passa a usar `pipeline_ctx` em call_cto, call_engineer, call_pm (inputs vindos de `build_inputs_*` quando disponível); _run_monitor_loop recebe `pipeline_ctx`, registra artefatos após QA_PASS e passa ao call_dev `task_dict`, `dependency_code` e `pipeline_ctx`; call_dev aceita `task_dict`, `dependency_code`, `pipeline_ctx` para montar inputs com current_task e contexto seletivo; testes em test_runtime_build_user_message.py. **005:** PM com depends_on_files obrigatório por task (formato e gate no SYSTEM_PROMPT); Monitor com bloco "Contexto seletivo" (depends_on_files e escalação); testes em test_pipeline_context.py (get_dependency_code, register_artifact). **006 (12 Leis):** LEI 1 — `AGENT_TEMPERATURE` no runtime (0-1 em `messages.create()`); `extract_thinking()` no envelope + log no runtime; LEI 12 — QA com gate explícito de ceticismo sobre código gerado por IA; testes extract_thinking. **007 (LEI 2):** Regras críticas no início e no fim do system prompt (lost in the middle): `SYSTEM_PROMPT_CRITICAL_RULES_LEI2.md` em contracts; `build_system_prompt()` prependa/append o bloco; teste em test_runtime_build_user_message. **008 (LEI 3):** `calculate_token_budget(system_msg, user_msg, model)` antes de cada chamada; MODEL_LIMITS por modelo; warning >60%, error >80%; `max_tokens = min(env, safe_max_tokens)`; testes em test_runtime_build_user_message. **009 (LEI 4):** `resilient_json_parse(raw_text, request_id)` com 3 níveis (parse direto, placeholder em `content`, fallback FAIL); `_extract_double_quoted`; `parse_response_envelope` usa resilient parse; regra 3.3 escaping em SYSTEM_PROMPT_PROTOCOL_SHARED; testes em test_envelope. **010 (LEI 5):** Retry com feedback estruturado: `build_repair_feedback_block(failed_response, validation_errors)`; run_agent usa o bloco no repair (nunca reenvia prompt idêntico); teste em test_runtime_build_user_message. **011 (LEI 6):** Conteúdo do usuário em `<user_provided_content>` em build_user_message (spec_raw) com aviso anti-injection; seção 1.2 do protocolo compartilhado reforçada (LEI 6); teste em test_runtime_build_user_message. **012 (LEI 7 e 8):** LEI 7 — get_dependency_code com MAX_TOTAL_DEPENDENCY_CHARS e _extract_interfaces para arquivos >20K; LEI 8 — regra no PM (máx. 3 arquivos por task), validate_backlog_tasks_max_files, log no Monitor Loop; testes em test_pipeline_context. **013 (LEI 9 e 10):** LEI 9 — TaskStateMachine (VALID_TRANSITIONS, rework_count, BLOCKED após max reworks) em task_state_machine.py; LEI 10 — log_agent_call(agent_name, mode, budget, response, duration_ms) em runtime, chamado em todos os retornos de run_agent; testes em test_task_state_machine e test_runtime_build_user_message. **014 (LEI 11):** Pipeline resumível — PipelineContext.current_step, save_checkpoint/load_checkpoint; testes em test_pipeline_context. Runner: ao iniciar tenta load_checkpoint(STATE_DIR); fases 1–3 só rodam se current_step < N; após cada fase current_step = N e save_checkpoint(STATE_DIR). **015:** Checklist das 12 Leis (§12.2 do AGENT_LLM_COMMUNICATION_ANALYSIS) marcado como concluído no próprio documento; resumo em [015](ia/015_20260221_checklist_12_leis_concluido.md). **016:** Teste E2E do pipeline — [tests/e2e/](../../tests/e2e/) com spec [project/spec/spec_landing_zentriz.txt](../spec/spec_landing_zentriz.txt), guia [E2E_PIPELINE_TEST_GUIDE.md](../docs/E2E_PIPELINE_TEST_GUIDE.md); registro em [016](ia/016_20260221_e2e_pipeline_landing.md). Contexto: [001](ia/001_20260221_plano_regras_llm.md) … [016](ia/016_20260221_e2e_pipeline_landing.md).

---

*Criado em 2026-02-17 — Zentriz Genesis. Atualize quando houver mudanças relevantes no estado do projeto. Seção 6 atualizada em 2026-02-20 (circuit breaker, gate Dev, relatório e docs). Regras LLM em 2026-02-21.*
