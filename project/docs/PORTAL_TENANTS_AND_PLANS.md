...
**Financeiro (RFC-0002 Parte B — F1):** nova tela `Financeiro` (`/zentriz/finance`), visível apenas para `zentriz_admin`, com abas Resumo (KPIs + gerar cobranças do mês), Cobranças (criar/cancelar/registrar pagamento), Pagamentos e Contas bancárias. Valores em BRL; dinheiro em centavos. Ao assinar (signup), o tenant nasce `inactive` com a 1ª cobrança de assinatura em aberto (H2).

**Financeiro (RFC-0002 Parte B — F2, entregue):** ciclo de vida da assinatura ligado ao caixa.
- **Ativação por pagamento:** registrar o pagamento que quita uma cobrança **de assinatura** (`kind = 'subscription'`) reativa o tenant (`inactive`/`suspended` → `active`), desde que não reste nenhuma assinatura vencida. Quitar cobrança avulsa (one_off/setup) **não** ativa o tenant nem reverte uma suspensão manual do master. A resposta de `POST /api/finance/payments` inclui `tenantActivated: boolean`.
- **Vencimento + suspensão (job periódico):** cobranças `open`/`partially_paid` com `due_date` no passado (fuso America/Sao_Paulo) viram `overdue`; tenant `active` com assinatura `overdue` além da carência (`FINANCE_SUSPEND_GRACE_DAYS`, default 3 dias) é `suspended`. A reativação só acontece por pagamento (anti-flapping).
- **Gate de inadimplência (H3):** além do bloqueio no login, cada requisição de usuário de tenant revalida o status; um token ainda válido cujo tenant foi suspenso/inativado no meio da sessão recebe `403 { code: "TENANT_INACTIVE" }`. Isentos: `zentriz_admin` (master) e os callbacks de máquina do runner (token `svc:"runner"`, RFC H1 — pipeline em voo nunca é derrubado). Kill-switch de emergência: `H3_TENANT_STATUS_GATE=off`.

**Financeiro (RFC-0002 Parte B — F3, entregue — MVP interno):** nova aba **Notas fiscais** na tela `Financeiro`. A partir de uma cobrança **paga**, o master emite uma nota interna pela ação "Emitir nota fiscal" (aba Cobranças); a nota herda tenant/valor/competência/descrição da cobrança, recebe um número sequencial próprio e uma referência do provedor (`INT-000123`). Cada cobrança tem no máximo **uma** nota emitida (dupla emissão é bloqueada); cancelar a nota libera reemissão. **Não** há integração com NFS-e municipal nem certificado nesta fase — a emissão passa por uma porta plugável (`InvoiceProvider`) cujo adaptador atual é um stub interno; a troca por um provedor real (F4) não altera o portal.
