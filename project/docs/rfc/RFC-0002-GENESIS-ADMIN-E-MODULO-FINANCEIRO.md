...
- **F4 (opcional):** porta `PaymentGateway` real (Asaas/Mercado Pago) + webhook de baixa (idempotente, L2); `InvoiceProvider` real + modelagem fiscal NFS-e completa (M1: ISS/retenções/`service_code`/emitente).
  - **Direção de emissão fiscal (decisão do Jean, 2026-08-17 — ver [ADR-023](../../../../docs/01-ecosystem/adr/ADR-023-FISCAL-DOCUMENT-EMISSION-AS-SEPARATE-PRODUCT.md) no workspace):** a emissão de documentos fiscais eletrônicos (NFS-e municipal/nacional e, futuramente, a família SEFAZ — NF-e/NFC-e/CT-e/MDF-e) será um **produto Zentriz separado**, consumido pelo Genesis **via API/MessageBus**. A porta `InvoiceProvider` é o ponto de desacoplamento: o segundo adaptador aponta primeiro para um **provider terceiro** (Focus NFe/eNotas/PlugNotas/NFe.io — ponte de curto prazo, emissão válida rápida) e, quando o produto fiscal Zentriz existir, passa a apontar para ele — **sem tocar nas rotas nem no portal**. Requer credenciais + certificado A1 (motivo do adiamento). Primeiro projeto do produto fiscal: **NFS-e**.
