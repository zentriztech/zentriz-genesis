...
|---|---|---|
| **D1** | Reparar agora os 28 `file_path` do Venuxx V2 (P0)? | ⏸️ adiado de manhã → ✅ **REABERTO E DECIDIDO no mesmo dia**: *"Materializar arquivo mínimo e corrigir o path"*. Não é um defeito de prefixo: `specs/<título>.md` nunca existiu em disco (semeado por `seed-venuxx-v2.ts:197`). ⚠️ **Correção de premissa (medida depois, na hora de executar):** o **conteúdo original existe** em `/project-files/<projectId>/docs/spec_<título>.md` — **28/28 presentes**, 358–444 bytes, cabeçalho `<!-- Created by: spec -->`; e `content_sha256` é **NULL** nas 28 linhas (isto sim inviabiliza o casamento por sha do P0). Como o banco é mais pobre que esse arquivo (`charter_summary` **é** o "## Objetivo" dele e `backlog_summary` é um contador, "1 módulo, 2 tarefas"), o script `src/db/backfill-venuxx-v2-spec-files.ts` (dry-run por default) foi reescrito para **copiar a spec semeada verbatim** e acrescentar um apêndice que **declara procedência e as lacunas L-01…L-07**; o esqueleto derivado do inventário ficou apenas como fallback (**não acionado**: dry-run em prod = 28/28 pelo caminho da spec semeada, 1.618–1.742 chars). Corrige `file_path`/`filename`/`content_sha256`/`spec_ref` para o formato canônico do `projectCreation.ts`. Consequência aceita: esses 28 projetos vão gerar GAPs verdadeiros — a lacuna é real e fica visível em vez de silenciosa. |
