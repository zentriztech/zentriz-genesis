...
- **Financeiro (RFC-0002 Parte B — F1)**: exclusivo de `zentriz_admin` (o Financeiro é função da conta de gestão; o guard de autoria NÃO se aplica). Dinheiro sempre em centavos inteiros; moeda única `BRL` nesta fase. Endpoints sob `/api/finance`:
  ...
  - **Pagamentos (baixa manual)**: `GET /api/finance/payments?tenantId=&chargeId=`; `POST` (body: `chargeId`, `amountCents>0`, `method∈{pix,boleto,card,transfer,cash,manual}`, `receivedAt?`, `bankAccountId?`, `reference?`, `notes?`) — recalcula o status da cobrança (único escritor); só aceita cobrança em `open|overdue|partially_paid`.
